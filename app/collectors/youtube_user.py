...
class YouTubeUserCollector:
    ...
    async def _paginate(self, endpoint: str, params: dict[str, Any], max_items: int | None = None) -> list[dict[str, Any]]:
        """Follow every page. A None cap means all pages; the legacy 5000 value is treated as unlimited."""
        if max_items == 5000:
            max_items = None
    ...
    async def playlists(self, max_items: int | None = None) -> list[dict[str, Any]]:
        return await self._paginate("playlists", {"part": "snippet,contentDetails", "mine": "true"}, max_items)

    async def playlist_items(self, playlist_id: str, max_items: int | None = None) -> list[dict[str, Any]]:
        return await self._paginate("playlistItems", {"part": "snippet,contentDetails", "playlistId": playlist_id}, max_items)

    async def subscriptions(self, max_items: int | None = None) -> list[dict[str, Any]]:
        return await self._paginate("subscriptions", {"part": "snippet,contentDetails", "mine": "true"}, max_items)
    ...
    async def owned_playlist_video_items(self, max_playlists: int | None = None) -> list[dict[str, Any]]:
        ...
        for playlist in await self.playlists(max_playlists):
            ...
            if not playlist_id:
                continue
            for item in await self.playlist_items(playlist_id):
    ...
    async def music_video_details_from_user_sources(self) -> tuple[list[dict[str, Any]], dict[str, int]]:
        ...
        return music, {"liked_items": len(liked), "playlist_items": len(playlist_items), "unique_videos": len(set(x for x in ids if x)), "videos_hydrated": len(details), "music_videos": len(music)}
